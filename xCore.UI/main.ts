...
const coin = { identity: 'x42', tooltip: 'xCore' };
...
ipcMain.on('start-daemon', (event, arg: Chain) => {
  ...
  if (arg.mode === 'manual') {
    ...
    const msg = 'xCore was started in development mode. This requires the user to be running the daemon manually.';
    writeLog(msg);
...
function shutdownDaemon(callback) {
  ...
  if (!hasDaemon) {
    writeLog('xCore is in mobile mode, no daemon to shutdown.');
    callback(true, null);
